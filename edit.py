...
if __name__ == "__main__":
    ...
    while True :
        ...
        if load_flag:   # If the current image to display is not loaded to the RAM yet
            img = load_image(IMAGES_PATH + image_path)
            pygame.display.set_caption(image_path)
            # Load the image
            load_flag = False
            zoom_config = [(1.0, (0.0, 0.0))]
            if image_path in data:
                circles = [ dejsonify(circle) for circle in data[image_path] ]
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYUP:
                if event.key == pygame.K_RETURN:
                    ...
                    data[image_path] = [jsonify(circle) for circle in circles]
                    save_curr_circles()
                    sys.exit()
                ...
                if event.key == pygame.K_UP:
                    ...
                    img = load_image(IMAGES_PATH + image_path, zoom_config=zoom_config)
